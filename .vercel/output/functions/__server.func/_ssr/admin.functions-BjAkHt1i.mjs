...
//#region node_modules/.nitro/vite/services/ssr/assets/admin.functions-BjAkHt1i.js
...
var upsertProduct = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => productInput.parse(input)).handler(createSsrRpc("87057b699c92d5e9cc4f021e767a44009606525481c8a27886c5fc0375dc6625"));
var deleteProduct = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => objectType({ id: stringType().uuid() }).parse(input)).handler(createSsrRpc("d58e9647e4364d7098cb0d840b98a827719a64c5b316ad80080e28b97fb3f1cb"));
...
var markOrderPaid = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => objectType({ id: stringType().uuid() }).parse(input)).handler(createSsrRpc("8ad2b16f17ba58b435e9fe5f2a2446ad1bcf5abece494589cb22056cd07c00bd"));
