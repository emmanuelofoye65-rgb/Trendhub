...
//#region node_modules/.nitro/vite/services/ssr/assets/admin.functions-C8t2lv4-.js
...
var upsertProduct = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => productInput.parse(input)).handler(upsertProduct_createServerFn_handler, async ({ context, data }) => {
	await assertAdmin(context);
	if (data.id) {
...
var deleteProduct = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({ id: stringType().uuid() }).parse(input)).handler(deleteProduct_createServerFn_handler, async ({ context, data }) => {
	await assertAdmin(context);
...
var markOrderPaid = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({ id: stringType().uuid() }).parse(input)).handler(markOrderPaid_createServerFn_handler, async ({ context, data }) => {
	await assertAdmin(context);
